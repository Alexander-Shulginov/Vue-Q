...
<code v-pre>v-bind</code> - односторонняя привязка реактивныхданных к DOM элементу<br>
...
Если функция вызывается с одними и теме же параметрами, вместо вычислений, происходит возврат закешированнго значения.</p>
<p>Директива позволяет сохранить результат вычислений между рендерингами, если значения не изменились элемент не будет перерисован.</p>
<h3 id="v-cloak" tabindex="-1"><a class="header-anchor" href="#v-cloak"><span>v-cloak</span></a></h3>
<p>Скрывает неотредеренный шаблон, удаляется с компонента когда он будет скомпилирован.</p>
