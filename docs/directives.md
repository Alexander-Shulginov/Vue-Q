...
Если функция вызывается с одними и теме же параметрами, вместо вычеслений, происходит возврат закешированнго значения.  

Директива позволяет сохранить результат вычеслений между рендерингами, если значения не изменились элемент не будет перерисован.  
